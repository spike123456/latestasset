...
app.controller('latestController', function($scope,$http) {
	...
	.then(function(response) {
		...
		if (gifts.includes(detailId)) {
			$('#more-info > tbody').append('<tr><td width="35"><div class="hand-right"></div></td><td>Hãy tiến hành đặt hàng ngay sau đó vào trang <a href="" id="wheel">Quay số trúng thẻ cào</a> để tìm cho mình phần thưởng nhé. Chúc các bạn may mắn</td></tr>');
		}
	});
});
